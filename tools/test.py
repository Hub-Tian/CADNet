import os
import torch
print("cuda device status", torch.cuda.is_available(), torch.cuda.device_count())
...
def parge_config():
    ...
    parser.add_argument('--expand', type=str, default='2', help='multi-scale type')
...
def main():
    ...
    test_set, test_loader, sampler = build_dataloader(
        cfg.DATA_CONFIG.DATA_DIR, args.batch_size, dist_test, workers=args.workers, logger=logger, training=False, expand=args.expand,
    )
    model = build_network(test_set,expand=args.expand)
    with torch.no_grad():
        if args.eval_all:
