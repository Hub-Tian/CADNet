...
def clean_data_new(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck']
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
    MAX_TRUNCATION = [0.15, 0.3, 0.5]
    ...
    for i in range(num_gt):
        ...
        if (gt_name == current_cls_name):
            ...
            valid_class = -1
        ignore = False
        if (dis<MAX_DIS[difficulty] or dis>MAX_DIS[difficulty+1]):
            # if gt_anno["difficulty"][i] > difficulty or gt_anno["difficulty"][i] == -1:
            ignore = True
        if valid_class == 1 and not ignore:
            ignored_gt.append(0)
    ...
    for i in range(num_dt):
        if (dt_anno["name"][i].lower() == current_cls_name):
            ...
            valid_class = -1
        height = abs(dt_anno["bbox"][i, 3] - dt_anno["bbox"][i, 1])
        dis = np.linalg.norm(dt_anno["location"][i])
        abs(dt_anno["bbox"][i, 3] - dt_anno["bbox"][i, 1])
        if (dis<MAX_DIS[difficulty] or dis>MAX_DIS[difficulty+1]):
            ignored_dt.append(1)
        elif valid_class == 1:
            ignored_dt.append(0)
