...
class DatasetTemplate(torch_data.Dataset):
    ...
    def prepare_data(self, input_dict, has_label=True, expand=None):
        """
        :param input_dict:
            sample_idx: string
        ...
        try:
            if expand == '2':
                voxels, voxels_expand, coordinates, num_points, num_points_expand = voxel_grid
                # print(voxels,voxels_expand)
            else:
            ...
            if expand == '2':
                voxels_expand = voxel_grid["voxels_expand"]
                num_points_expand = voxel_grid["num_points_per_voxel_expand"]
        ...
        if expand == '2':
            example.update({
                'voxels_expand': voxels_expand,
                'num_points_expand': num_points_expand
            })
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, elems in example_merged.items():
            if key in ['voxels', 'voxels_expand', 'num_points', 'num_points_expand', 'voxel_centers', 'seg_labels',
